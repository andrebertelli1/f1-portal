        ...
        <div key={topParticipant.id} className="flex items-center">
          <Avatar className="h-9 w-9">
            {topParticipant.pilot?.photoUrl ? (
              <AvatarImage className="object-cover" src={topParticipant.pilot.photoUrl} alt="Avatar" />
            ) : (
              <AvatarFallback>{getInitials(topParticipant.pilot?.name ?? '')}</AvatarFallback>
            )}
          </Avatar>
          <div className="ml-4 space-y-1">
            <p className="text-sm font-medium leading-none">{topParticipant.pilot?.name ?? "N/A"}</p>
